...
public class Combat_Bat : MonoBehaviour
{
    ...
    // Character Animator
    private MoveBat enemyMoveScript;
    ...
    void Start()
    {
        PlayerHealthHUD = FindObjectOfType<HealthSlider2D_Script>().GetComponent<UnityEngine.UI.Slider>();
        ...
        enemyMoveScript = GetComponent<MoveBat>();
        ...
        if (targetedEnemy != null)
        {
            if (enemyAttackType == EnemyAttackType.Melee)
            {
                ...
                if (performMeleeAttack && enemyWithinAttackRange)
                {
                    ...
                    //Debug.Log(GetComponent<EnemyStatsScript>().enemyHealth);
                    Invoke("InvBomb", 2.5f);
                    // Start Courotine
