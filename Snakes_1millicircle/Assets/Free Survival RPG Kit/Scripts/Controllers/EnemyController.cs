...
public class EnemyController : MonoBehaviour {
	...
	void Start()
	{
		target = Player23.instance.transform;
	...
	void Update ()
	{
		...
		if (distance <= lookRadius)
		{
			...
			if (distance <= agent.stoppingDistance)
			{
                ...
                // Check Player23 Health
                if (Player23.instance.playerStats.currentHealth > 0)
                {
                    // Attack               
                    combatManager.Attack(Player23.instance.playerStats);
                    FaceTarget();
                }
                else
                {
