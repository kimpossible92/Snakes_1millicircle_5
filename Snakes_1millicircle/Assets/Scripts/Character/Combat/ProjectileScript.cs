...
public class ProjectileScript : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (setFreeze&& projAbilityTypeString== "Targeted_Q") { transform.position = setIsPosition; }
    }
    bool setFreeze = false;
    Vector3 setIsPosition;
    private void Update()
    {
        rb.AddRelativeForce(move * 10 - rb.velocity);
        if (Vector3.Distance(transform.position, FindObjectOfType<TargetableScript>().transform.position) <= 0.3f)
        {
            if(FindObjectOfType<TargetableScript>().enemyType == TargetableScript.EnemyType.Minion)
            {
                setFreeze = true;
                setIsPosition = FindObjectOfType<TargetableScript>().transform.position;print("freeze");
            }
        }
    }
    ...
    public void DealDamage(EnemyStatsScript enemyStats) // TODO - Switch damagetype and defense type based on damage type - attack, ability, true (ignore def & res)
    {
        if (hitObject == false)
        {
            ...
            GameObject go = Instantiate(DamageTextPopup_object, transform.position + new Vector3(Random.Range(-1f, 1f), Random.Range(0, 0), 0), Quaternion.identity);
