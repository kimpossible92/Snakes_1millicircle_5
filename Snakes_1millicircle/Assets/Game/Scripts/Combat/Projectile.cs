...
namespace RPG.Combat
{
    public class Projectile : MonoBehaviour
    {
        ...
        {
            if (target == null) return;
            bool isDead = target.GetComponent<Health3>().IsDead();
        ...
        private void OnTriggerEnter(Collider other)
        {
            ...
            if (other.gameObject.GetComponent<Health3>() != null)
            {
                other.GetComponent<Health3>().TakeDamage(damage);
                //handle last arrow that hits and kills enemy
                if (other.GetComponent<Health3>().IsDead())
                {
                    Destroy(gameObject);
                    return;
                }
            ...
            else if (other.gameObject == target.gameObject && !target.GetComponent<Health3>().IsDead())
            {
                //do damage
                other.GetComponent<Health3>().TakeDamage(damage);
